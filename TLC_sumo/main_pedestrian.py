...
def ipa_gradient_method_pedestrian(initial_par, lam, demand_scale, step_size, par_update_step_size, run_time,
    ...
    while iter_num < total_iter_num:
        ...
        # update parameters
        par_update_step_size = get_par_update_step_size(par_update_step_size, [list(i) for i in zip(*d_L_list)], 1., 4.,
                                                        0.5, 1.5)
...
def update_result(d_L_list, performance_list, d_L, queue_length, depart_veh_num, depart_ped_num, par_list,
    ...
    # update parameters
    par_update_step_size = get_par_update_step_size(30, [list(i) for i in zip(*d_L_list)], 1., 4., 0.5, 1.5)
    print("par_update_step_size: " + str(par_update_step_size))
    # theta_1_min
    par_list[0].append(max(0.1, par_list[0][-1] - par_update_step_size * d_L[0]))
    # theta_1_max
    ...
    return theta, d_L_list, performance_list, par_list


def update_par_check(queue_length, step_per_iter):
    update_par_flag = False
    if step_per_iter < 300:
        return update_par_flag

    tmp_pre = sum([sum(l[-120:-60]) for l in queue_length])
    tmp_cur = sum([sum(l[-60:]) for l in queue_length])  # total queue num for last minute
    if (tmp_cur-tmp_pre)/tmp_pre > 0.3:
...
def ped_adaptive_sequential(initial_par, step_size, run_time, print_mode):
    ...
    while step < run_time:
        ...
        if update_par_flag:
            ...
            theta, d_L_list, performance_list, par_list = update_result(d_L_list, performance_list, d_L, queue_length, \
                                                                        depart_veh_num, depart_ped_num, par_list,
                                                                        step_per_iter, print_mode)
...
if __name__ == "__main__":
    ...
    ipa_gradient_method_pedestrian_sequential(initial_par=[10, 20, 30, 50, 10, 10, 8, 8, 8, 8],
                                              ...
                                              demand_scale=1.6, step_size=1, run_time=36000, iter_num=1,
                                              print_mode=False)
