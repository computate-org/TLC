...
def one_iter(theta_1_min, theta_1_max, theta_2_min, theta_2_max, s_1, s_2, lam_1, lam_2, run_time, fix_seed, print_mode,
    ...
    traci.start(
        [sumoBinary, "-c", "Veberod_intersection.sumocfg", "--fcd-output.geo", "true", "--fcd-output",
         "veberod-fcd.xml",
    ...
    while step < run_time:
        ...
        if print_mode:
            ...
            print("veh_ids: " + str(traci.lanearea.getLastStepVehicleIDs("det_13")) + ">>>>" + str(
                traci.lanearea.getLastStepVehicleIDs("det_42")))
        ...
        if green_1:
            ...
            # x2 = s2(b)
                ...
                if alpha_2_rate == 0:
                    ...
                    d_tau = (np.array([0, 0, 0, 0, 0, 1]) - np.array(d_x2)) / alpha_2_rate
                if print_mode:
                    print("x2 = s2(b) G2R1")
        ...
        if print_mode:
            ...
            print('===========================================================================================')
    try:
        mean_waiting_time_total = (sum(queue_length_1[100:]) + sum(queue_length_2[100:])) / (
...
def ipa_gradient_method(initial_par, lam_1, lam_2, run_time, iters_per_par, total_iter_num, stepsize, sumoBinary,
                        print_mode):
...
def brute_force_method(initial_par, lam_1, lam_2, run_time, iters_per_par, total_iter_num, par_change_idx, stepsize,
